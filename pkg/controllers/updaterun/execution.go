...
func (r *Reconciler) execute(
	...
	if updatingStageIndex < len(updateRun.Status.StagesStatus) {
		...
		if updatingErr := r.recordUpdateRunStatus(ctx, updateRun); updatingErr != nil {
			klog.ErrorS(updatingErr, "Failed to update the clusterStagedUpdateRun status", "clusterStagedUpdateRun", klog.KObj(updateRun))
		}
	...
	for i := range updatingStageStatus.Clusters {
		...
		if !condition.IsConditionStatusTrue(clusterStartedCond, updateRun.Generation) {
			...
			if !isBindingSyncedWithClusterStatus(updateRun, binding, clusterStatus) {
				...
				klog.V(2).InfoS("Find the first binding that is updating but the cluster status has not been updated", "cluster", clusterStatus.ClusterName, "stage", updatingStageStatus.StageName, "clusterStagedUpdateRun", updateRunRef)
				if binding.Spec.State != placementv1beta1.BindingStateBound {
					...
					if _, updateErr := checkClusterUpdateResult(availCond, binding, clusterStatus, updatingStageStatus, updateRun); updateErr != nil {
		...
		finished, updateErr := checkClusterUpdateResult(availCond, binding, clusterStatus, updatingStageStatus, updateRun)
		if finished {
			finishedClusterCount++
			continue
		}
	...
	for _, binding := range toBeDeletedBindings {
		...
		if isBindingSyncedWithClusterStatus(updateRun, binding, curCluster) {
			...
			if err := r.Client.Delete(ctx, binding); err != nil {
				klog.ErrorS(err, "Failed to delete a binding in the update run", "clusterResourceBinding", klog.KObj(binding), "cluster", curCluster.ClusterName, "clusterStagedUpdateRun", updateRunRef)
				return false, controller.NewAPIServerError(false, err)
			}
			klog.V(2).InfoS("Deleted a binding pointing to a to be deleted cluster", "clusterResourceBinding", klog.KObj(binding), "cluster", curCluster.ClusterName, "clusterStagedUpdateRun", updateRunRef)
			...
			klog.ErrorS(delErr, "The binding has been changed after the updating, please check if there is concurrent update run", "clusterStagedUpdateRun", updateRunRef)
...
func (r *Reconciler) checkAfterStageTasksStatus(ctx context.Context, updatingStageIndex int, updateRun *placementv1alpha1.ClusterStagedUpdateRun) (bool, error) {
	...
	if updatingStage.AfterStageTasks == nil {
		klog.V(2).InfoS("There is no after stage task for this stage", "stage", updatingStage.Name, "stagedUpdateRun", updateRunRef)
		return true, nil
	}
	for i, task := range updatingStage.AfterStageTasks {
		...
		case placementv1alpha1.AfterStageTaskTypeTimedWait:
			...
			if waitStartTime.Add(task.WaitTime.Duration).After(time.Now()) {
				klog.V(2).InfoS("The after stage task still need to wait", "waitStartTime", waitStartTime, "waitTime", task.WaitTime, "stage", updatingStage.Name, "stagedUpdateRun", updateRunRef)
				return false, nil
			}
			markAfterStageWaitTimeElapsed(&updatingStageStatus.AfterStageTaskStatus[i], updateRun.Generation)
			klog.V(2).InfoS("The after stage wait task has completed", "stage", updatingStage.Name, "stagedUpdateRun", updateRunRef)
			...
			if err := r.Client.Create(ctx, &approvalRequest); err != nil {
				if apierrors.IsAlreadyExists(err) {
					...
					if err = r.Client.Get(ctx, client.ObjectKeyFromObject(&approvalRequest), &approvalRequest); err != nil {
						klog.ErrorS(err, "Failed to get the already existing approval request", "approvalRequest", requestRef, "stage", updatingStage.Name, "stagedUpdateRun", updateRunRef)
						return false, err
					}
					if approvalRequest.Spec.TargetStage != updatingStage.Name || approvalRequest.Spec.TargetUpdateRun != updateRun.Name {
						...
						klog.ErrorS(unexpectedErr, "Found an approval request targeting wrong stage", "approvalRequestTask", requestRef, "stage", updatingStage.Name, "stagedUpdateRun", updateRunRef)
						return false, fmt.Errorf("%w: %s", errStagedUpdatedAborted, unexpectedErr.Error())
					}
					if !condition.IsConditionStatusTrue(meta.FindStatusCondition(approvalRequest.Status.Conditions, string(placementv1alpha1.ApprovalRequestConditionApproved)), approvalRequest.Generation) {
						klog.V(2).InfoS("The approval request has not been approved yet", "approvalRequestTask", requestRef, "stage", updatingStage.Name, "stagedUpdateRun", updateRunRef)
						return false, nil
					}
					klog.V(2).InfoS("The approval request has been approved", "approvalRequestTask", requestRef, "stage", updatingStage.Name, "stagedUpdateRun", updateRunRef)
					markAfterStageRequestApproved(&updatingStageStatus.AfterStageTaskStatus[i], updateRun.Generation)
				} else {
					// retryable error
					klog.ErrorS(err, "Failed to create the approval request", "approvalRequest", requestRef, "stage", updatingStage.Name, "stagedUpdateRun", updateRunRef)
					return false, err
				}
			} else {
				// the approval request has been created for the first time
				klog.V(2).InfoS("The approval request has been created", "approvalRequestTask", requestRef, "stage", updatingStage.Name, "stagedUpdateRun", updateRunRef)
				markAfterStageRequestCreated(&updatingStageStatus.AfterStageTaskStatus[i], updateRun.Generation)
				return false, nil
			}
		}
...
func checkClusterUpdateResult(
	...
	for i := condition.OverriddenCondition; i < condition.AppliedCondition; i++ {
		...
		if condition.IsConditionStatusFalse(bindingCond, binding.Generation) {
			...
			klog.InfoS("The cluster updating encountered an error", "failedCondition", bindingCond, "cluster", clusterStatus.ClusterName, "stage", updatingStage.StageName, "stagedUpdateRun", klog.KObj(updateRun))
			// TODO(wantjian): identify some none recoverable error and mark the cluster updating as failed
