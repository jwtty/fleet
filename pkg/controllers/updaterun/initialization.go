	...
	for i, binding := range bindingList.Items {
		if binding.Spec.SchedulingPolicySnapshotName == latestPolicySnapshot.Name {
			if binding.Spec.State != placementv1beta1.BindingStateScheduled {
				// can retry and see if the binding is scheduled later?
...
func (r *Reconciler) generateStagesByStrategy(
	...
	if err := r.Client.Get(ctx, client.ObjectKey{Name: updateRun.Spec.StagedUpdateStrategyName}, &updateStrategy); err != nil {
		...
		if apierrors.IsNotFound(err) {
			...
			return fmt.Errorf("%w: %s", errInitializedFailed, "Referenced clusterStagedUpdateStrategy "+updateRun.Spec.StagedUpdateStrategyName+" not found")
		}
...
func (r *Reconciler) computeRunStageStatus(
		...
		for _, cluster := range clusterList.Items {
			if _, ok := allSelectedClusters[cluster.Name]; ok {
				if _, ok := allPlacedClusters[cluster.Name]; ok {
					...
					dupErr := fmt.Errorf("cluster `%s` appears in more than one stage", cluster.Name)
	...
	// Check if the clusters in the stage are all placed.
...
func validateAfterStageTask(tasks []placementv1alpha1.AfterStageTask) error {
	if len(tasks) > 2 {
		return fmt.Errorf("afterStageTasks should have at most 2 tasks, 1 waiting task and 1 approval task")
	}
...
func (r *Reconciler) recordOverrideSnapshots(ctx context.Context, updateRun *placementv1alpha1.ClusterStagedUpdateRun) error {
	...
	if len(masterResourceSnapshot.Annotations[placementv1beta1.ResourceGroupHashAnnotation]) == 0 {
		nomasterErr := fmt.Errorf("the resource snapshot is not a master snapshot")
